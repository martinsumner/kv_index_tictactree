...
handle_call({put, Object, IndexN, OtherVnodes}, _From, State) ->
    ...
    {UpdClock, PrevClock} =
        ...
        of
            not_found ->
                {[{State#state.vnode_id, State#state.vnode_sqn}], none};
            {ok, Head} ->
                Clock0 =
...
handle_call({rebuild, true}, _From, State) ->
    ...
    case
        aae_controller:aae_rebuildstore(
            State#state.aae_controller,
            SplitFun
        )
    of
        ...
        {ok, FoldFun, FinishFun} ->
            ...
            % dispatch the work to the worker
            Worker(Runner, FinishFun)
    end,
...
handle_call({aae, Msg, IndexNs, ReturnFun}, _From, State) ->
    ...
    {reply, ok, State};
handle_call(
    {fold_aae, Range, Segments, FoldFun, InitAcc, Elements},
    _From,
    State
) ->
    R = aae_controller:aae_fold(
        State#state.aae_controller,
        Range,
        Segments,
        FoldFun,
...
extractclock_from_riakhead(
    <<?MAGIC:8/integer, ?V1_VERS:8/integer, VclockLen:32/integer,
        VclockBin:VclockLen/binary, _Rest/binary>>
) ->
...
aaefold_setmodifiedlimiter({LowModDate, HighModDate}) when
    is_integer(LowModDate), is_integer(HighModDate)
->
...
to_aae_binary(ObjectBin) ->
    ...
    <<?MAGIC:8/integer, ?V1_VERS:8/integer, VclockLen:32/integer,
        _VclockBin:VclockLen/binary, SibCount:32/integer,
        SibsBin/binary>> = ObjectBin,

    % faking here
    IndexHash = erlang:phash2([]),

    {{LMDmeg, LMDsec, LMDmcr}, MD} =
        strip_metabinary(SibCount, SibsBin, {0, 0, 0}, <<>>),

    <<ObjectSize:32/integer, SibCount:32/integer, IndexHash:32/integer,
        LMDmeg:32/integer, LMDsec:32/integer, LMDmcr:32/integer, MD/binary>>.

strip_metabinary(0, <<>>, LMD, MetaBinAcc) ->
    {LMD, MetaBinAcc};
strip_metabinary(SibCount, SibBin, LMD, MetaBinAcc) ->
    <<ValLen:32/integer, _ValBin:ValLen/binary, MetaLen:32/integer,
        MetaBin:MetaLen/binary, Rest/binary>> = SibBin,
    ...
    strip_metabinary(
        ...
        Rest,
        LMD0,
        <<MetaBinAcc/binary, MetaLen:32/integer, MetaBin:MetaLen/binary>>
    ).
