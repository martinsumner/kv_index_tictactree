...
mock_vnode_loadexchangeandrebuild_tester(TupleBuckets, PType) ->
    ...
    io:format(
        "Rehash some entries and confirm root_compare " ++
    ...
    io:format(
        "Compare the two stores using an AAE fold - " ++
    ...
    io:format(
        "Looking for Segment IDs K1 ~w ~w K2 ~w ~w~n",
    ...
    io:format(
        "Exchange between nodes to expose differences" ++
    ...
    io:format(
        "Discover Next rebuild times - should be in the future " ++
    ...
    io:format(
        "Should still discover the same difference " ++
    ...
    {ok, _P3mr1, GUID3mr1} =
        aae_exchange:start(
            ...
            NullRepairFun,
            ReturnFun,
            {filter, all, all, large, all, {MRH - (60 * 60), MRH}, pre_hash},
            []
        ),
    ...
    {ok, _P3mr2, GUID3mr2} =
        aae_exchange:start(
            ...
            {filter, all, all, large, all,
                {MRH - (2 * 60 * 60), MRH - (60 * 60)}, pre_hash},
            []
        ),
    ...
    {ok, _P3bmr1, GUID3bmr1} =
        aae_exchange:start(
            ...
            NullRepairFun,
            ReturnFun,
            {filter, Bucket1, all, large, all, {MRH - (60 * 60), MRH},
                pre_hash},
            []
        ),
    ...
    {ok, _P3bmr2, GUID3bmr2} =
        aae_exchange:start(
            ...
            NullRepairFun,
            ReturnFun,
            {filter, Bucket2, all, large, all, {MRH - (60 * 60), MRH},
                pre_hash},
            []
        ),
    ...
    {ok, _P3bmr3, GUID3bmr3} =
        aae_exchange:start(
            ...
            NullRepairFun,
            ReturnFun,
            {filter, Bucket, all, large, all, {MRH - (60 * 60), MRH}, pre_hash},
            []
        ),
    ...
    true = RebuildPc == RebuildPa,

    io:format(
        "Following a completed rebuild - the exchange should still" ++
    ...
    CheckBucketFun =
        fun(CheckBucket, Acc) ->
            ...
            io:format(
                "~w differences found in bucket ~w~n",
                [CBN, CheckBucket]
            ),
            Acc + CBN
        end,
                ...
                SubRepairFun =
                    fun({{RepB, K}, _VCCompare}, Acc) ->
                        case lists:keyfind(K, 1, RepairList) of
                            ...
                            false ->
                                io:format(
                                    "Missing from repair list ~w ~w~n",
                                    [RepB, K]
                                ),
                                Acc
                        end
    ...
    io:format(
        "Repaired ~w from list of length ~w~n",
    ...
    io:format(
        "StartKey ~s EndKey ~s in Range test~n",
    ...
    io:format(
        "Total range repairs after key range test ~w~n",
        [TotalRepairsKR3]
    ),
    ...
    io:format(
        "Exchange in second modified range resulted in ~w~n",
        [TS3_4_Result]
    ),
    ...
    io:format(
        "Exchange in first modified range resulted in ~w~n",
        [TS1_2_Result]
    ),
        ...
        fun(_I, Acc) ->
            case
                RepairBucketFun(
                    ...
                    RepairFunMR3,
                    all,
                    {convert_ts(MDR_TS1), convert_ts(os:timestamp())},
                    pre_hash
                )
            of
                {clock_compare, Count3} ->
    ...
    io:format(
        "Total range repairs after modified range test ~w~n",
        [TotalRepairsMR3]
    ),
...
mock_vnode_coveragefolder(Type, InitialKeyCount, TupleBuckets) ->
    ...
    {SC1, SibL1} = Folder3(),
    io:format(
        "Coverage fold took ~w with output ~w for store ~w~n",
    ...
    DedupedBL = lists:usort(
        BucketListF1() ++ BucketListF2() ++
...
fold_metabin(<<>>, MDAcc) ->
    lists:reverse(MDAcc);
fold_metabin(
    <<0:32/integer, MetaLen:32/integer, MetaBin:MetaLen/binary, Rest/binary>>,
    MDAcc
) ->
...
fold_metabin(
    <<MetaLen:32/integer, MetaBin:MetaLen/binary, Rest/binary>>, MDAcc
) ->
    <<_LastModBin:12/binary, VTagLen:8/integer, _VTagBin:VTagLen/binary,
        _Deleted:1/binary-unit:8, MetaDictBin/binary>> = MetaBin,
    fold_metabin(Rest, [binary_to_term(MetaDictBin) | MDAcc]).
