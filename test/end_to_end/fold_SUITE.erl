...
aae_fold_tester(ParallelStoreType, KeyCount) ->
    ...
    {async, Runner2} =
        aae_controller:aae_fold(
            ...
            {buckets, BucketList},
            all,
            {SWLowMegaS * 1000000 + SWLowS, SWHighMegaS * 1000000 + SWHighS},
            false,
            FoldFun,
            InitAcc,
            FoldElements
    ...
    BKVSL_ByBL =
        lists:filter(
            fun({B, _K, _V}) -> lists:member(B, BucketList) end,
            BKVSL
        ),
    ...
    io:format(
        "Fetched ~w clocks with segment filter~n",
