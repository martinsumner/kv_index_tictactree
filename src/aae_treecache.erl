...
-spec cache_open(
    list(), partition_id(), aae_util:log_levels()
) -> {boolean(), pid()}.
%% @doc
...
-spec cache_replacedirtysegments(
    pid(),
...
handle_cast(start_load, State = #state{loading = Loading}) when
    Loading == false
->
    {noreply, State#state{
        loading = true,
...
handle_cast({complete_load, Tree}, State = #state{loading = Loading}) when
    Loading == true
->
    LoadFun =
    ...
    UpdateHash =
        case {CurrentHash, OldHash} of
            ...
            {none, none} ->
                ...
                AltKeyHash;
            {CurrentHash, OldHash} when
                is_integer(CurrentHash), is_integer(OldHash)
            ->
...
setup_savedcaches(RootPath) ->
    ...
    Tree1 = leveled_tictac:add_kv(
        Tree0,
        {<<"K1">>},
        {<<"V1">>},
        fun({K}, {V}) -> {K, V} end
    ),
    Tree2 = leveled_tictac:add_kv(
        Tree1,
        {<<"K2">>},
        {<<"V2">>},
        fun({K}, {V}) -> {K, V} end
...
replace_test() ->
    ...
    CompareTree1 = DirectAddFun(
        {<<"K_With0Hash">>, (1 bsl 27) - 1}, CompareTree
    ),
